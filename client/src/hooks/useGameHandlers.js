 ...
 */
export function useGameHandlers(socketApi, roomId, playerId, setError) {
  const { emit } = socketApi ?? {};

  const handleCreateRoom = useCallback(({ roomId: id, playerName: name }) => {
    if (!emit) return;
    setError(null);
    emit("createRoom", { roomId: id, playerId, playerName: name });
  }, [emit, playerId, setError]);

  const handleJoinRoom = useCallback(({ roomId: id, playerName: name }) => {
    if (!emit) return;
    setError(null);
